...
{pmore}{ul:"pairs" format} - a data set of pairwise comparisons.
Here, each two-arm study is represented by a single record in the usual way, 
but a multi-arm study is represented by one record for {hi:every} possible pairwise contrast in that study. 
...
{pmore}A common error is to omit some pairwise contrasts. 
For example, a 3-arm trial must be represented by 3 records, 
and a 4-arm trial must be represented by 6 records.

{pmore}{ul:"augmented" format} - a data set of contrasts with a common reference treatment.
...
{pstd}Use data stored at {browse "https://clinicalepidemio.fr/Stata/example_datasets.zip"}:
