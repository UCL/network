...
{pstd}
{cmd:misspattern} creates a rectangular display of a data set, 
with the variables on the vertical axis and observations on the horizontal axis, 
filled in with two different colours to show observed and missing data. 
It can be used to assess the extent and pattern of missing data in a dataset.
...
{synoptline}
{syntab:Main}
{synopt:{opt novarsort}}Keeps variables in their order in the data set. 
Otherwise the variables are sorted from least missing at the top to most missing at the bottom.{p_end}
{synopt:{opt noidsort}}Keeps observations in their original order in the data set. 
Otherwise the observations are sorted from least missing at the left to most missing at the right.{p_end}
...
{synopt:{opt clear}}clears the current data and loads the summary data into memory: 
this could be useful if you want to fine-tune the graph{p_end}
...
{synopt:{cmd:varlabel(}{help cat_axis_label_options}{opt )}}allows options for the appearance of the variable names or labels. 
For example, varlabel(labsize(small) labcol(red)).{p_end}
...
The code uses one option to define the colour of each bar: because Stata limits the number of options to 70, 
the code fails with the message "too many options" if there are more than about 60 bars.
If this happens, try reducing the number of variables. 
(In the future I may find a way to eliminate a few observations so that the code works on the remainder.)
