...
foreach trtcode in `trtcodelist' {
	if !mi("`trtname`trtcode''") local trtnames `"`trtnames' "`trtname`trtcode''""' 
	else local trtnames `"`trtnames' "`trtcode'""' 
}
...
global F9 networkplot `_trt'1 `_trt'2, `newoptions' `options' 
di "networkplot command is stored in F9"
di "About to pause"
pause
di "Finished pause"
